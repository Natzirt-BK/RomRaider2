...
| 11 | User Level menu is missing. | The packaged shell now has a five-level User Level menu, marks the active level, persists changes, and refreshes the calibration list using the compatibility visibility rule. The user accepted filtering against actual level 1, 4, and 5 tables in the Linux candidate. | User level is visible and persistent, and changes expose/hide the same calibration scope as the compatibility application. | LINUX PASS; WINDOWS RETEST |
| 12 | Settings menu is missing. | The packaged shell now has a top-level Settings menu that opens the retained comprehensive Editor settings window; the user accepted the repaired Linux candidate. Logger setup remains separate. | All retained desktop settings are reachable, understandable, persistent, and separated from Logger connection setup. | LINUX PASS; WINDOWS RETEST |
