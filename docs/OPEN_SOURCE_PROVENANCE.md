...
| Adopted in part | NikolaKozina j2534 | https://github.com/NikolaKozina/j2534 | BSD-3-Clause | OpenPort 2.0 command framing for Android's native USB host transport; existing Linux J2534 implementation | The Android path independently uses `UsbDeviceConnection`; the x86/glibc Linux binary is not packaged into Android. The upstream command framing, USB identity, acknowledgements, firmware and battery queries, ISO9141 channel setup, and vehicle-message wrapper are retained with the BSD notice in `licenses/NikolaKozina-j2534-BSD-3-Clause.txt`. Android exposes read-only requests; hardware support and test limits are documented in the Android guide. |
...
| Adopted | Compose Multiplatform for Desktop | https://github.com/JetBrains/compose-multiplatform | Apache-2.0; bundled Skia is BSD-3-Clause | Replacement Logger workspace on Windows x64 and Linux x64 | Compose 1.12.0, Kotlin 2.4.10, and Skiko 0.150.1 are locked and hash-verified. Platform packages contain only their matching native renderer. The Apache and Skia licenses plus a component notice ship in `licenses/`. |
...
| Field | Compose Desktop adoption record |
| --- | --- |
| Project and repository | Compose Multiplatform, https://github.com/JetBrains/compose-multiplatform |
| Revision | Compose 1.12.0, Kotlin 2.4.10, Skiko 0.150.1 |
| License | Apache-2.0 for the JVM libraries; BSD-3-Clause for Skia embedded in the Skiko native runtime |
| Upstream files | Unmodified Maven runtime artifacts resolved by `org.jetbrains.compose.desktop.currentOs` for Windows x64, Linux x64, macOS ARM64, and macOS x64 |
| RomRaider2 files | `ui/compose-logger`, `settings.gradle.kts`, `build.gradle.kts`, `gradle/verification-metadata.xml`, `ui/compose-logger/gradle.lockfile`, and the Compose/Apache/Skia files in `licenses/` |
| Reuse type | Unmodified runtime dependencies with a RomRaider2-owned Logger workspace and Java Service Provider bridge |
| Modifications | No upstream modifications. Runtime jars are renamed with their Maven group while staging so artifacts with the same base filename remain distinct. Each package includes only its matching native Skiko renderer. |
| Attribution | The complete Apache-2.0 and Skia BSD-3-Clause texts and a component/version notice are shipped in each desktop platform package. Embedded upstream license files remain intact. |
| Validation | Locked dependency graph and SHA-256 verification metadata; Kotlin unit tests; ServiceLoader discovery; Linux Xvfb visual checks at normal, narrow, light, and dark layouts; Linux package inspection. Windows and both native macOS package/visual checks remain release gates. |
| Reviewer/date | RomRaider2 project audit, 2026-08-31 |
