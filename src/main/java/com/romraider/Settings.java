...
public class Settings implements Serializable {
    ...
    /* RomRaider2 platform selection (schema version is written by the XML builder). */
    ...
    public Settings() {
        // Center the window when a display is available. Keeping deterministic
        // defaults in headless mode makes settings migration testable in CI.
    ...
    public void setJavaFxCellSize(Dimension size) {
        javaFxCellSize = new Dimension(Math.max(60, Math.min(500, size.width)),
                Math.max(26, Math.min(120, size.height)));
    }
    ...
    public final void setLocale(String locale) {
        ...
        if (!locale.equalsIgnoreCase(SYSTEM_NUMFORMAT)) {
            ...
            final Locale lc = new Locale.Builder()
                    .setLanguage(language[0])
                    .setRegion(language[1])
                    .build();
    ...
    public VehiclePlatform getVehiclePlatform() {
        return vehiclePlatform;
    }

    public void setVehiclePlatform(VehiclePlatform vehiclePlatform) {
        if (vehiclePlatform == null) {
            throw new IllegalArgumentException("Vehicle platform is required");
        }
        this.vehiclePlatform = vehiclePlatform;
    }
    ...
    public void setUiScale(UiScale uiScale) {
        if (uiScale == null) {
            throw new IllegalArgumentException("UI scale is required");
        }
    ...
    public void setDisplayMode(DisplayMode displayMode) {
        if (displayMode == null) {
            throw new IllegalArgumentException("Display mode is required");
        }
        this.displayMode = displayMode;
    }

    public EditorWorkspacePreferences getEditorWorkspacePreferences() {
        return editorWorkspacePreferences;
    }

    public void setEditorWorkspacePreferences(
            EditorWorkspacePreferences editorWorkspacePreferences) {
        if (editorWorkspacePreferences == null) {
            throw new IllegalArgumentException("Editor workspace preferences are required");
        }
        this.editorWorkspacePreferences = editorWorkspacePreferences;
    }
}
