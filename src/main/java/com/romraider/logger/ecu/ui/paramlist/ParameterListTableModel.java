...
public final class ParameterListTableModel extends AbstractTableModel {
    ...
    public synchronized int findRowById(String parameterId) {
        if (parameterId == null) return -1;
        for (int row = 0; row < registeredLoggerData.size(); row++) {
            LoggerData data = registeredLoggerData.get(row);
            if (parameterId.equals(data.getId())) return row;
        }
        return -1;
    }
