...
public final class DynoControlPanel extends JPanel {
    ...
    private static final String NO_CAR_PROFILES = rb.getString("NOPROFILES");
    ...
    private String[] carTypeArr = {NO_CAR_PROFILES};
    private boolean carDefinitionsLoaded;
    ...
    private JToggleButton buildRecordDataButton() {
        if (carDefinitionsLoaded) {
            ...
            recordDataButton.setToolTipText(rb.getString("CARFILEHELP"));
        }
    ...
    private void loadFromFile() {
        final JFileChooser openFile = new IntegratedFileChooser();
    ...
    private JComboBox buildCarSelectComboBox() {
        ...
        selectComboBox.setEnabled(carDefinitionsLoaded);
    ...
    private JComboBox buildGearComboBox() {
        ...
        gearSelectBox.setEnabled(carDefinitionsLoaded);
        gearSelectBox.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent actionEvent) {
                int carIndex = carSelectBox.getSelectedIndex();
                int gearIndex = gearSelectBox.getSelectedIndex();
                if (!hasGearSelection(carDefinitionsLoaded, gearsRatioArr,
                        carIndex, gearIndex)) return;
                gearRatio.setText(gearsRatioArr[carIndex][gearIndex + 1]);
    ...
    static boolean hasGearSelection(boolean definitionsLoaded,
            String[][] ratios, int carIndex, int gearIndex) {
        if (!definitionsLoaded || ratios == null || carIndex < 0
                || carIndex >= ratios.length || gearIndex < 0) return false;
        String[] carRatios = ratios[carIndex];
        return carRatios != null && gearIndex + 1 < carRatios.length
                && carRatios[gearIndex + 1] != null;
    }
    ...
    private void loadCars() {
        try {
            ...
            if (totalCars == 0) {
                throw new IOException("cars_def.xml contains no vehicle profiles");
            }
            ...
            useNoCarProfilesFallback();
            LOGGER.warn("No usable " + CARS_FILE
                    + " file found; Dyno recording remains disabled.", exception);
        }
    }

    private void useNoCarProfilesFallback() {
        carDefinitionsLoaded = false;
        carTypeArr = new String[]{NO_CAR_PROFILES};
    }
