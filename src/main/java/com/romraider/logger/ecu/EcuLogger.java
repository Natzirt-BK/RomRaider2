...
/*
TODO: Continue separating Logger session state from this Swing frame.
TODO: Allow tab-specific settings such as Dashboard warning levels in profiles.
 */
...
public final class EcuLogger extends AbstractFrame implements EcuRelatedMessageListener {
    ...
    private JTabbedPane dataParameterTabs;
    private JTable dataParameterTable;
    private JTable dataSwitchTable;
    private JTable dataExternalTable;
    ...
    private final LoggerParameterFocusService.Listener parameterFocusListener =
            new LoggerParameterFocusService.Listener() {
        public boolean focusRequested(String parameterId) {
            return focusLoggerDataItem(parameterId);
        }
    };
    ...
    private void construct() {
        ...
        Configurator.setRootLevel(Level.toLevel(
                getSettings().getLoggerDebuggingLevel()));
        ...
        if (ecuEditor == null) {
            ...
            initDataUpdateHandlers();
            startPortRefresherThread();
            ecuEditor.getStatusPanel().ready(rb.getString("READY"));
        }
        LoggerParameterFocusService.getInstance().addListener(
    ...
    private void bootstrap() {
        ...
        controller.addListener(new FileLoggingConnectionMonitor(
                fileUpdateHandler, new Runnable() {
                    @Override
                    public void run() {
        ...
        tabbedPane.setName("LOGGER WORKSPACE TABS");
    ...
    private void initControllerListeners() {
        ...
        controller.addListener(LoggerLiveDataBus.getInstance());
    }

    private void startPortRefresherThread() {
        if (usesJ2534Interface()) {
            return;
        }
    ...
    private void showMissingConfigDialog() {
        ...
        LOGGER.warn("Logger definition file not configured; parameter logging disabled");
    }
    ...
    private JButton buildUnselectAllButton() {
        final JButton button = new JButton("Clear Selections");
        SetFont.plain(button);
        button.setName("LOGGER CLEAR SELECTIONS");
    ...
    private JButton buildLtvButton() {
        final JButton button = new JButton("Learning Tables");
        SetFont.plain(button);
        button.setName("LOGGER LEARNING TABLES");
        button.setToolTipText("Open learning-table values (F6)");
    ...
    private void addSplitPaneTab(String name, final JSplitPane splitPane, JComponent... extraControls) {
        ...
        toggleListButton.setName("TOGGLE LOGGER PARAMETERS");
        ...
        JPanel tabControlPanel = new JPanel(new BetterFlowLayout(
                FlowLayout.LEFT, 6, 2));
        tabControlPanel.setName("LOGGER WORKSPACE ACTIONS");
        tabControlPanel.setBorder(BorderFactory.createEmptyBorder(4, 6, 4, 6));
        tabControlPanel.add(toggleListButton);
        if (splitPane.getLeftComponent() instanceof LoggerParameterBrowser) {
            ((LoggerParameterBrowser) splitPane.getLeftComponent()).setActions(
                    extraControls);
        } else {
            for (JComponent control : extraControls) {
                tabControlPanel.add(control);
            }
        }

        JPanel panel = new JPanel(new BorderLayout(0, 0));
        panel.add(tabControlPanel, NORTH);
    ...
    private JComponent buildParamListPane(ParameterListTableModel paramListTableModel, ParameterListTableModel switchListTableModel, ParameterListTableModel externalListTableModel) {
        ParameterListTable parameterTable = buildParamListTable(
                paramListTableModel);
        ParameterListTable switchTable = buildParamListTable(
                switchListTableModel);
        ParameterListTable externalTable = buildParamListTable(
                externalListTableModel);

        if (paramListTableModel == dataTabParamListTableModel) {
            dataParameterTable = parameterTable;
            dataSwitchTable = switchTable;
            dataExternalTable = externalTable;
        }

        LoggerParameterBrowser browser = new LoggerParameterBrowser(
                new ParameterListTableModel[] {paramListTableModel,
                        switchListTableModel, externalListTableModel},
                new ParameterListTable[] {parameterTable, switchTable,
                        externalTable},
                new String[] {HEADING_PARAMETERS, HEADING_SWITCHES,
                        "External Sensors"});
        if (paramListTableModel == dataTabParamListTableModel) {
            dataParameterTabs = browser.getSectionTabs();
        }
        return browser;
    }

    private ParameterListTable buildParamListTable(
            ParameterListTableModel tableModel) {
        ParameterListTable paramListTable = new ParameterListTable(tableModel);
    ...
    private JComponent buildStatusBar() {
        ...
        statusBar.setName("LOGGER STATUS BAR");
        ...
        statusBar.add(messagePanel, CENTER);

        JPanel ecuIdPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 0));
        ecuIdPanel.add(calIdLabel);
        ecuIdPanel.add(dmLabel);
        ecuIdPanel.add(ecuIdLabel);
        ecuIdPanel.add(statsLabel);
        statusBar.add(ecuIdPanel, EAST);
    ...
    private JSplitPane buildSplitPane(JComponent leftComponent, JComponent rightComponent) {
        ...
        splitPane.setName("LOGGER WORKSPACE SPLIT");
        splitPane.setDividerSize(7);
        splitPane.setContinuousLayout(true);
        splitPane.setOneTouchExpandable(true);
        splitPane.setResizeWeight(0.24);
        leftComponent.setMinimumSize(new Dimension(260, 180));
        rightComponent.setMinimumSize(new Dimension(480, 240));
    ...
    private JPanel buildControlToolbar() {
        ...
        controlPanel.setName("LOGGER COMMAND AREA");

        JPanel commandBar = new JPanel(new BorderLayout(12, 0));
        commandBar.setName("LOGGER COMMAND BAR");
        commandBar.setBorder(BorderFactory.createEmptyBorder(6, 10, 6, 10));
        JLabel brand = new JLabel(
                "RomRaider2 Logger",
                BrandImages.icon(32), JLabel.LEFT);
        brand.setName("LOGGER BRAND");
        brand.setIconTextGap(8);
        commandBar.add(brand, WEST);
        ...
        commandBar.add(buildPortsComboBox(), CENTER);
        commandBar.add(buildStatusIndicator(), EAST);
        controlPanel.add(commandBar, NORTH);

        JPanel contextBar = new JPanel(new BorderLayout(10, 0));
        contextBar.setName("LOGGER VIEW CONTROLS");
        ...
        JLabel context = new JLabel(
                "Configure channels, connect to the ECU, then start recording");
        context.setForeground(UiThemeService.getInstance().color(
                ThemeToken.SECONDARY_TEXT));
        contextBar.add(context, WEST);
        contextBar.add(new DisplayPreferencesPanel(), EAST);
        controlPanel.add(contextBar, SOUTH);
        return controlPanel;
    }
    ...
    private Component buildFileNameExtention() {
        ...
        fileNameExtention.setName("LOGGER LOG TITLE");
        if (touchEnabled == true)
        {
    ...
    private final Component buildLogToFileButton() {
        ...
        logToFileButton.setName("LOGGER FILE RECORDING");
    ...
    private void resetFileLoggingButton() {
    ...
    private void styleFileLoggingButton(boolean recording) {
        java.awt.Color background = UiThemeService.getInstance().color(
                recording ? ThemeToken.DANGER : ThemeToken.SUCCESS);
        logToFileButton.setBackground(background);
        logToFileButton.setForeground(UiThemeService.contrastText(background));
    }

    private JPanel buildPortsComboBox() {
        final boolean j2534Interface = usesJ2534Interface();
        if (j2534Interface) {
            portsComboBox.showJ2534Interface();
        }
        else {
        ...
        JPanel comboBoxPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 6, 0));
        comboBoxPanel.setName("LOGGER CONNECTION CONTROLS");
        comboBoxPanel.add(new JLabel(j2534Interface ? "Interface:" : rb.getString("COMPORT")));
        comboBoxPanel.add(portsComboBox);

        JButton connectionSetup = new JButton("Vehicle / Module…");
        connectionSetup.setName("LOGGER CONNECTION SETUP");
        connectionSetup.setToolTipText(
                "Choose the vehicle and control module before connecting");
        connectionSetup.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent actionEvent) {
                showConnectionSetup();
            }
        });
        comboBoxPanel.add(connectionSetup);

        JButton reconnectButton = new JButton("Connect",
                new ImageIcon(getClass().getResource("/graphics/logger_restart.png")));
        componentList.put("reconnectButton", reconnectButton);
        if (touchEnabled == false)
        {
            reconnectButton.setPreferredSize(new Dimension(104, 34));
        }
        else
        {
        ...
        JButton disconnectButton = new JButton("Disconnect",
                new ImageIcon(getClass().getResource("/graphics/logger_stop.png")));
        componentList.put("disconnectButton", disconnectButton);
        if (touchEnabled == false)
        {
            disconnectButton.setPreferredSize(new Dimension(118, 34));
        }
        else
        {
    ...
    private boolean usesJ2534Interface() {
        return !isNullOrEmpty(getSettings().getJ2534Device()) ||
                !isNullOrEmpty(System.getProperty("romraider2.j2534.library"));
    }
    ...
    private void setTarget(String name) {
        ...
        final String currentLabel = ecuIdLabel.getText();
        final int valueSeparator = currentLabel.indexOf(": ");
        final String ecuId = valueSeparator >= 0
                ? currentLabel.substring(valueSeparator + 2) : "Unknown";
        ecuIdLabel.setText(buildEcuInfoLabelText(target + " ID", ecuId));
    ...
    private JComponent buildDataTab() {
        return new LoggerLiveDataPanel(dataTableModel,
                new Runnable() {
                    public void run() { liveDataUpdateHandler.reset(); }
                });
    }
    ...
    private JComponent buildDashboardTab() {
        final LoggerDashboardPanel panel = new LoggerDashboardPanel(
                dashboardPanel,
                new Runnable() {
                    public void run() { dashboardUpdateHandler.reset(); }
                },
                new Runnable() {
                    public void run() {
        ...
        panel.getStyleButton().getInputMap(WHEN_IN_FOCUSED_WINDOW).put(
                getKeyStroke("F12"), "toggleGaugeStyle");
        panel.getStyleButton().getActionMap().put("toggleGaugeStyle",
                new AbstractAction() {
                    private static final long serialVersionUID = 1L;
                    public void actionPerformed(ActionEvent event) {
                        panel.getStyleButton().doClick();
                    }
                });
        return panel;
    }
    ...
    public void showConnectionSetup() {
        ...
        VehiclePlatform previousPlatform = context.getPlatform();
        VehicleModule previousModule = context.getModule();
        Settings settings = getSettings();
        String previousProtocol = settings.getLoggerProtocol();
        String previousTransport = settings.getTransportProtocol();
        String previousTarget = settings.getTargetModule();
        JPanel panel = new JPanel(new BorderLayout(0, 10));
        panel.add(new JLabel("Choose the vehicle and module used for the next connection."),
                NORTH);
        panel.add(new PlatformSelectorPanel(), CENTER);
        int answer = showConfirmDialog(this, panel,
                "Logger Connection Setup", OK_CANCEL_OPTION, PLAIN_MESSAGE);
        if (answer != OK_OPTION) {
            restorePlatformSelection(context, previousPlatform, previousModule);
            return;
        }

        VehiclePlatform selectedPlatform = context.getPlatform();
        String desiredProtocol = selectedPlatform == VehiclePlatform.EVO_8_9
                ? "MUT2" : "SSM";
        if (!protocolList.isEmpty() && !protocolList.containsKey(desiredProtocol)) {
            restorePlatformSelection(context, previousPlatform, previousModule);
            showMessageDialog(this,
                    "The installed Logger definition does not provide "
                    + desiredProtocol + ". Install a compatible definition first.",
                    "Logger Connection Setup", WARNING_MESSAGE);
            return;
        }

        settings.setVehiclePlatform(selectedPlatform);
        settings.setVehicleModule(context.getModule());
        settings.setLoggerProtocol(desiredProtocol);
        settings.setTransportProtocol("ISO9141");
        settings.setTargetModule(loggerModuleId(context.getModule()));
        try {
            ...
            restorePlatformSelection(context, previousPlatform, previousModule);
            settings.setLoggerProtocol(previousProtocol);
            settings.setTransportProtocol(previousTransport);
            settings.setTargetModule(previousTarget);
            try {
                SettingsManager.save(settings);
            } catch (Exception rollbackFailure) {
                exception.addSuppressed(rollbackFailure);
            }
            reportError("Unable to save Logger connection selection", exception);
            return;
        }
        reportMessage("Connection setup: " + selectedPlatform + " • "
                + context.getModule() + " • " + desiredProtocol);
    }

    private void restorePlatformSelection(PlatformContext context,
            VehiclePlatform platform, VehicleModule module) {
        context.setPlatform(platform);
        context.setModule(module);
        getSettings().setVehiclePlatform(platform);
        getSettings().setVehicleModule(module);
    }

    private static String loggerModuleId(VehicleModule module) {
        if (module == VehicleModule.TCU) return "tcu";
        if (module == VehicleModule.AYC_ACD) return "ayc";
        if (module == VehicleModule.ABS) return "abs";
        return "ecu";
    }
    ...
    /** Brings the existing logger window forward without creating a duplicate. */
    ...
    private boolean focusLoggerDataItem(String parameterId) {
        LoggerDataLocation location = findLoggerDataItem(parameterId);
        if (location == null) return false;
        final LoggerDataLocation targetLocation = location;
        invokeLater(new Runnable() {
            public void run() {
                selectTab(0);
                if (dataParameterTabs != null) {
                    dataParameterTabs.setSelectedIndex(targetLocation.section);
                }
                int viewRow = targetLocation.table.convertRowIndexToView(
                        targetLocation.modelRow);
                if (viewRow >= 0) {
                    targetLocation.table.getSelectionModel()
                            .setSelectionInterval(viewRow, viewRow);
                    targetLocation.table.scrollRectToVisible(
                            targetLocation.table.getCellRect(viewRow, 1, true));
                }
                showAndFocus();
            }
        });
    ...
    private LoggerDataLocation findLoggerDataItem(String parameterId) {
        ParameterListTableModel[] models = {dataTabParamListTableModel,
                dataTabSwitchListTableModel, dataTabExternalListTableModel};
        JTable[] tables = {dataParameterTable, dataSwitchTable,
                dataExternalTable};
        for (int section = 0; section < models.length; section++) {
            if (models[section] == null || tables[section] == null) continue;
            int modelRow = models[section].findRowById(parameterId);
            if (modelRow >= 0) {
                return new LoggerDataLocation(section, modelRow,
                        tables[section]);
            }
        }
        return null;
    ...
    private static final class LoggerDataLocation {
        private final int section;
        private final int modelRow;
        private final JTable table;

        private LoggerDataLocation(int section, int modelRow, JTable table) {
        ...
        if (message != null) {
            invokeLater(new Runnable() {
                ...
                public void run() {
                    ...
                    messageLabel.setForeground(UiThemeService.getInstance().color(
    ...
    public void setRefreshMode(boolean refreshMode) {
        ...
        if (refresher != null) refresher.setRefreshMode(refreshMode);
    }
