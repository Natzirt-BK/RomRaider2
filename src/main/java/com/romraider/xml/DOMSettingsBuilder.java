...
public final class DOMSettingsBuilder {
    ...
    public void buildSettings(Settings settings, File output, JProgressPane progress, String versionNumber) throws IOException {
        ...
        settingsNode.appendChild(buildPlatformSelection(settings));
        settingsNode.appendChild(buildDisplayPreferences(settings));
        settingsNode.appendChild(buildEditorWorkspace(settings));
    ...
    private IIOMetadataNode buildPlatformSelection(Settings settings) {
        IIOMetadataNode platform = new IIOMetadataNode("platform-selection");
        platform.setAttribute("schema", "1");
        platform.setAttribute("vehicle", settings.getVehiclePlatform().name());
        platform.setAttribute("module", settings.getVehicleModule().name());
        return platform;
    }

    private IIOMetadataNode buildDisplayPreferences(Settings settings) {
        IIOMetadataNode display = new IIOMetadataNode("display-preferences");
        display.setAttribute("schema", "1");
        display.setAttribute("scale", settings.getUiScale().name());
        display.setAttribute("theme", settings.getThemeMode().name());
        display.setAttribute("mode", settings.getDisplayMode().name());
        return display;
    }

    private IIOMetadataNode buildEditorWorkspace(Settings settings) {
        IIOMetadataNode workspace = new IIOMetadataNode("editor-workspace");
        workspace.setAttribute("schema", "3");
        EditorWorkspacePreferences preferences = settings.getEditorWorkspacePreferences();

        IIOMetadataNode favorites = new IIOMetadataNode("favorites");
        for (TableLocation location : preferences.getFavorites()) {
            favorites.appendChild(buildTableLocation(location));
        }
        workspace.appendChild(favorites);

        IIOMetadataNode recent = new IIOMetadataNode("recent");
        for (TableLocation location : preferences.getRecent()) {
            recent.appendChild(buildTableLocation(location));
        }
        workspace.appendChild(recent);

        for (Map.Entry<String, List<String>> entry
                : preferences.getAllOpenTables().entrySet()) {
            IIOMetadataNode open = new IIOMetadataNode("open");
            open.setAttribute("rom", entry.getKey());
            String active = preferences.getActiveTable(entry.getKey());
            if (active != null) open.setAttribute("active", active);
            for (String tableName : entry.getValue()) {
                IIOMetadataNode table = new IIOMetadataNode("table");
                table.setAttribute("name", tableName);
                open.appendChild(table);
            }
            workspace.appendChild(open);
        }

        IIOMetadataNode notes = new IIOMetadataNode("notes");
        for (Map.Entry<TableLocation, String> entry
                : preferences.getAllTableNotes().entrySet()) {
            IIOMetadataNode note = buildTableLocation(entry.getKey());
            note.setAttribute("text", entry.getValue());
    ...
    private IIOMetadataNode buildTableLocation(TableLocation location) {
        IIOMetadataNode table = new IIOMetadataNode("table");
        table.setAttribute("rom", location.getRomId());
        table.setAttribute("name", location.getTableName());
        return table;
    }
