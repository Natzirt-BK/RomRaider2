    ...
    public void populateTableVisual() {	
    	...
    	if(!isAxis() && !isHidden()) {
	        ...
	        if(null == table.name || table.name.isEmpty()) {
	            ...
	            tableLabel = createTableTitleLabel(getName());
	            add(createTableHeader(tableLabel,
	                    createTableMetadataLabel(table.getDataSize() + " values"),
	                    null), BorderLayout.NORTH);
	        } else {
	            tableLabel = createTableTitleLabel(getName() + " (" + table.getCurrentScale().getUnit() + ")");
	            add(createTableHeader(tableLabel,
	                    createTableMetadataLabel(table.getDataSize() + " values"),
	                    null), BorderLayout.NORTH);
	        }
