...
public class Table3DView extends TableView {
   ...
   // private static final ResourceBundle rb = new ResourceUtil().getBundle(Table3DView.class.getName());
    ...
    private TableColorScaleLegend colorScaleLegend;
    ...
    public void populateTableVisual() {	
        ...
        JLabel titleLabel = createTableTitleLabel(getName());
        ...
        styleAxisLabel(xAxisLabel);
        tableLabel = createTableMetadataLabel(table.getSizeX() + " × "
                + table.getSizeY() + "  •  " + tableUnit());
        add(createTableHeader(titleLabel, tableLabel, xAxisLabel),
                BorderLayout.NORTH);
        ...
        if(yAxisLabel!=null) {
	        ...
	        styleAxisLabel(yAxisLabel);
	        add(yAxisLabel, BorderLayout.WEST);
	        yAxisLabel.setBorder(new EmptyBorder(6, 8, 6, 8));
        }

        colorScaleLegend = new TableColorScaleLegend(table);
        add(colorScaleLegend, BorderLayout.SOUTH);
    ...
    public void updateTableLabel() {
        if(null == xAxis.getName() || xAxis.getName().length() < 1 || Settings.BLANK == xAxis.getName()) {
            ...
            // static or no scale exists.
            if (xAxisLabel != null) xAxisLabel.setText(xAxis.getName());
        } else {
            if (xAxisLabel != null) xAxisLabel.setText(xAxis.getName() + " (" + xAxis.getTable().getCurrentScale().getUnit() + ")");
        }
        ...
        if(null == yAxis.getName() || yAxis.getName().length() < 1 || Settings.BLANK == yAxis.getName()) {
            ...
            // static or no scale exists.
            if (yAxisLabel != null) yAxisLabel.setText(yAxis.getName());
        } else {
            if (yAxisLabel != null) yAxisLabel.setText(yAxis.getName() + " (" + yAxis.getTable().getCurrentScale().getUnit() + ")");
        }

        tableLabel.setText(table.getSizeX() + " × " + table.getSizeY()
                + "  •  " + tableUnit());
        if (colorScaleLegend != null) colorScaleLegend.repaint();
    }
    ...
    public void undoSelected() throws UserLevelException {
        ...
        if (xAxis != null) xAxis.undoSelected();
        if (yAxis != null) yAxis.undoSelected();
    }
