...
public class Table2DView extends TableView {
    ...
    public void populateTableVisual(){
        ...
        styleAxisLabel(axisLabel);
        tableLabel = createTableMetadataLabel(table.getDataSize()
                + " values  •  " + tableUnit());
        add(createTableHeader(createTableTitleLabel(getName()), tableLabel,
                axisLabel), BorderLayout.NORTH);
    ...
    public void updateTableLabel() {
        if (null == axis.getName() || axis.getName().length() < 1 || Settings.BLANK == axis.getName()) {
            ...
            // static or no scale exists.
            if (axisLabel != null) axisLabel.setText(axis.getName());
        } else {
            if (axisLabel != null) axisLabel.setText(axis.getName() + " ("
                    + axis.getTable().getCurrentScale().getUnit() + ")");
        }

        tableLabel.setText(table.getDataSize() + " values  •  "
                + tableUnit());
    ...
    public void undoSelected() throws UserLevelException {
        super.undoSelected();
        if (axis != null) axis.undoSelected();
    }
