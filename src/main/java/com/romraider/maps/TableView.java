...
public abstract class TableView extends JPanel implements Serializable {
    ...
    protected JLabel createTableTitleLabel(String text) {
        JLabel label = new JLabel(text);
        label.setFont(label.getFont().deriveFont(Font.BOLD,
                label.getFont().getSize2D() + 2.0f));
        label.setForeground(UiThemeService.getInstance().color(
                ThemeToken.PRIMARY_TEXT));
        return label;
    }

    protected JLabel createTableMetadataLabel(String text) {
        JLabel label = new JLabel(text);
        ...
        label.setBorder(BorderFactory.createCompoundBorder(
                UIManager.getBorder("RomRaider2.tableCellBorder"),
                BorderFactory.createEmptyBorder(3, 8, 3, 8)));
        return label;
    }

    protected void styleAxisLabel(JLabel label) {
    ...
    protected JPanel createTableHeader(JLabel title, JLabel metadata,
            JLabel axis) {
        JPanel header = new JPanel(new BorderLayout(12, 4));
        header.setName("TABLE VIEW HEADER");
        header.setBackground(UiThemeService.getInstance().color(
                ThemeToken.SURFACE));
        header.setBorder(BorderFactory.createEmptyBorder(8, 12, 7, 12));
        if (title != null) header.add(title, BorderLayout.WEST);
        if (metadata != null) header.add(metadata, BorderLayout.EAST);
        if (axis != null) header.add(axis, BorderLayout.SOUTH);
        tableHeader = header;
        return header;
    }
    ...
    public void setEmbeddedDocumentMode(boolean embedded) {
        ...
        Component metadata = layout.getLayoutComponent(BorderLayout.EAST);
        Component axis = layout.getLayoutComponent(BorderLayout.SOUTH);
        if (title != null) title.setVisible(!embedded);
        if (metadata != null) metadata.setVisible(!embedded);
        tableHeader.setBorder(embedded
                ? BorderFactory.createEmptyBorder(4, 8, 5, 8)
    ...
    protected JLabel createAxisCornerCell() {
        ...
        corner.setBackground(UiThemeService.getInstance().color(
                ThemeToken.RAISED_SURFACE));
        javax.swing.border.Border border = UIManager.getBorder(
                "RomRaider2.tableCellBorder");
        if (border != null) corner.setBorder(border);
        return corner;
    }

    protected String tableUnit() {
        if (table.getCurrentScale() == null
                || table.getCurrentScale().getUnit() == null
                || "0x".equals(table.getCurrentScale().getUnit())) {
            return "Raw";
        }
