...
public final class AboutAction extends AbstractAction {
    ...
    public void actionPerformed(final ActionEvent actionEvent) {
        final String message = MessageFormat.format(
                ...
                Version.FULL_PRODUCT_NAME,
                Version.TAGLINE,
                Version.VERSION,
                Version.BUILDNUMBER,
                Version.SUPPORT_URL,
                displayDefinitionVersion(logger.getDefVersion()),
                System.getProperty("java.vendor"),
                System.getProperty("java.runtime.version"),
                System.getProperty("os.arch"));
        final String title = MessageFormat.format(
                rb.getString("ABOUT"), Version.FULL_PRODUCT_NAME);
        showMessageDialog(logger, message, title,
                INFORMATION_MESSAGE, Version.ABOUT_ICON);
    }

    static String displayDefinitionVersion(final String version) {
        return version == null || version.trim().isEmpty()
                ? rb.getString("LOGGERDEFNOTINSTALLED") : version;
    }
}
