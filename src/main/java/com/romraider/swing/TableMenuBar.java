...
public class TableMenuBar extends JMenuBar {
    ...
    public TableMenuBar(TableFrame frame) {
        setBorder(javax.swing.BorderFactory.createEmptyBorder(1, 6, 1, 6));
        setName("Map context menu");
        fileMenu.setText("Map");
        initFileMenu(frame);
        initEditMenu(frame);
        initViewMenu(frame);
    ...
    private void initFileMenu(TableFrame frame) {
        ...
        graph.setIcon(ModernIconFactory.icon(Action.VIEW_3D));
        getClose().setIcon(ModernIconFactory.icon(Action.CLOSE));

        graph.setMnemonic('G');
        getClose().setMnemonic('X');
        graph.setEnabled(frame.getTable() instanceof Table3D);
        graph.setToolTipText("Show the integrated Java2D calibration surface");
    ...
    private void initViewMenu(TableFrame frame) {
        ...
        getTableProperties().setIcon(ModernIconFactory.icon(Action.PROPERTIES));
        getCompareOriginal().setIcon(ModernIconFactory.icon(Action.COMPARE));
        getCompareMap().setIcon(ModernIconFactory.icon(Action.COMPARE));
