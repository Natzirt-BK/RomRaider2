...
public class ECUEditorMenuBar extends JMenuBar implements ActionListener,
	...
	private final JMenuItem toggleInspector = new JMenuItem("Show or hide Inspector");
	...
	public ECUEditorMenuBar() {
		...
		viewMenu.add(toggleInspector);
		toggleInspector.addActionListener(this);
		toggleInspector.setMnemonic('I');
		...
		installModernIcons();
		updatePlatformActions(PlatformContext.getInstance());
	...
	public void platformContextChanged(PlatformContext context) {
		Runnable update = () -> updatePlatformActions(context);
		if (javax.swing.SwingUtilities.isEventDispatchThread()) update.run();
		else javax.swing.SwingUtilities.invokeLater(update);
	}

	private void updatePlatformActions(PlatformContext context) {
		boolean researchEnabled = Boolean.getBoolean(
				"romraider2.enableRamTuneResearch");
		boolean subaru = context.getPlatform() == VehiclePlatform.SUBARU;
		launchRamTuneTestApp.setVisible(researchEnabled && subaru);
		launchRamTuneTestApp.setEnabled(researchEnabled && subaru);
		launchRamTuneTestApp.setToolTipText(researchEnabled && subaru
				? "Developer-only DimeMod RAM Tune research tool"
				: "Realtime ECU writes are not enabled in RomRaider2");
		toolMenu.revalidate();
		toolMenu.repaint();
	}

	private void installModernIcons() {
		openImage.setIcon(ModernIconFactory.icon(Action.OPEN));
		quickSaveImage.setIcon(ModernIconFactory.icon(Action.SAVE));
		saveImage.setIcon(ModernIconFactory.icon(Action.SAVE_AS));
		saveAsRepository.setIcon(ModernIconFactory.icon(Action.SAVE));
		refreshImage.setIcon(ModernIconFactory.icon(Action.REFRESH));
		...
		toggleInspector.setIcon(ModernIconFactory.icon(Action.PROPERTIES));
		levelMenu.setIcon(ModernIconFactory.icon(Action.USER));
		openLogger.setIcon(ModernIconFactory.icon(Action.LOGGER));
		launchRamTuneTestApp.setIcon(ModernIconFactory.icon(Action.TOOLS));
		about.setIcon(ModernIconFactory.icon(Action.HELP));
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == openImage) {
			...
			showMessageDialog(parent, new RomPropertyPanel(parent.getLastSelectedRom()),
					...
					INFORMATION_MESSAGE);
		} else if (e.getSource() == toggleInspector) {
			parent.toggleInspector();
			...
			showMessageDialog(parent,
					MessageFormat.format(rb.getString("ABOUTMSG"), Version.FULL_PRODUCT_NAME, Version.TAGLINE,
							VERSION, BUILDNUMBER, SUPPORT_URL,
							System.getProperty("java.vendor"), System.getProperty("java.runtime.version"),
							System.getProperty("os.arch")),
					MessageFormat.format(rb.getString("ABOUT"), Version.FULL_PRODUCT_NAME), INFORMATION_MESSAGE, ABOUT_ICON);
		}
	}

	public void showCompareImagesDialog() {
		ECUEditor parent = ECUEditorManager.getECUEditor();
		parent.showCompareWorkspace();
	}
	...
	public void refreshImage() throws Exception {
		...
		if (parent.getLastSelectedRom() != null) {
			...
			if (parent.closeImage()) parent.openImage(file);
		}
	}

	public void openImageDialog() throws Exception {
		ECUEditor parent = ECUEditorManager.getECUEditor();
		JFileChooser fc = new IntegratedFileChooser(SettingsManager.getSettings().getLastImageDir());
	...
	public boolean saveImage(boolean quickSave) throws Exception {
		Rom lastSelectedRom = ECUEditorManager.getECUEditor().getLastSelectedRom();
		return saveImage(lastSelectedRom, quickSave);
	}

	public boolean saveImage(Rom rom, boolean quickSave) throws Exception {
		if (rom != null) {

			File selectedFile = rom.getFullFileName();
			if (!quickSave || selectedFile == null)
				selectedFile = getImageOutputFile(rom);

			if (null != selectedFile) {
				byte[] output = rom.saveFile();
				this.writeImage(rom, output, selectedFile);
				RomChangeService.markSaved(rom);
				RomRecoveryService.getInstance().markResolved(rom);
				ECUEditorManager.getECUEditor().refreshUI();
				return true;
			}
		}
		return false;
	...
	private File getImageOutputFile(Rom rom) throws Exception {
		ECUEditor parent = ECUEditorManager.getECUEditor();
		...
		if (fc.showSaveDialog(parent) == JFileChooser.APPROVE_OPTION) {
			...
			if (!selectedFile.getName().contains(".")) {
				String lastFile = rom.getFileName().toLowerCase();
				String format = ".bin";
	...
	private void writeImage(Rom rom, byte[] output, File selectedFile) throws Exception {
		ECUEditor parent = ECUEditorManager.getECUEditor();
		...
		rom.setFullFileName(selectedFile.getAbsoluteFile());
		parent.setLastSelectedRom(rom);
		SettingsManager.getSettings().setLastImageDir(selectedFile.getParentFile());
	}

	private File getRepositoryOutputDir() {
		JFileChooser fc = new IntegratedFileChooser();
	...
	private void increaseRomSize() throws Exception {
		...
		if (lastSelectedRom != null) {
			File selectedFile = getImageOutputFile(lastSelectedRom);
			if (null != selectedFile) {
				if (lastSelectedRom.getRealFileSize() != Settings.SIXTEENBIT_SMALL_SIZE) {
					...
					this.writeImage(lastSelectedRom, incOutput, selectedFile);
				}
			}
		}
	}
		...
		if (lastSelectedRom != null) {
			...
			if (null != selectedFile) {
				if (lastSelectedRom.getRealFileSize() != Settings.SIXTEENBIT_LARGE_SIZE) {
					...
					this.writeImage(lastSelectedRom, decOutput, selectedFile);
				}
			}
		}
	}
