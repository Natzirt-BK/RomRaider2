...
public class TableFrame extends JInternalFrame implements InternalFrameListener, ActionListener {
    ...
    private void updateToolbar(Table t) {
        ...
        if (parent.getTableToolBar() != null) parent.getTableToolBar().updateTableToolBar(t);
        if (parent.getToolBar() != null) parent.getToolBar().updateButtons();
        if (parent.getEditorMenuBar() != null) parent.getEditorMenuBar().updateMenu();
    }

    public void activateForWorkspace() {
    ...
    public void internalFrameActivated(InternalFrameEvent e) {
        if (!workspaceManaged) updateToolbar(getTable());
    }

    @Override
    public void internalFrameDeactivated(InternalFrameEvent e) {
        if (!workspaceManaged) updateToolbar(null);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == menu.getUndoAll()) {
            try (EditTransaction edit = RomEditHistory.getInstance().begin(
                    t, "Reset all map values")) {
                t.undoAll();
            }

        } else if (e.getSource() == menu.getRevert()) {
            t.setRevertPoint();
            RomEditHistory.getInstance().clear(t.getRom());

        } else if (e.getSource() == menu.getUndoSel()) {
            try (EditTransaction edit = RomEditHistory.getInstance().begin(
                    t, "Reset selected values")) {
