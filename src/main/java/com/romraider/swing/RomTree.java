...
public class RomTree extends JTree implements MouseListener {
    ...
    public RomTree(DefaultMutableTreeNode input) {
        ...
        setFont(baseFont == null ? new Font(Font.SANS_SERIF, Font.PLAIN, 12)
                : baseFont.deriveFont(Font.PLAIN,
                        Math.max(12.0f, baseFont.getSize2D())));
    ...
    public String getToolTipText(MouseEvent event) {
        ...
        if (value instanceof TableTreeNode) {
            TableTreeNode node = (TableTreeNode) value;
            String description = node.getToolTipText();
            if (description == null || description.trim().isEmpty()) {
                description = node.getTable().getDescription();
            }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (shouldOpenTable(e.getClickCount(),
                SettingsManager.getSettings().getTableClickCount())
    ...
    static boolean shouldOpenTable(int clickCount, int configuredClickCount) {
        return clickCount == configuredClickCount;
    }
