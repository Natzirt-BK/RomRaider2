...
public final class J2534Impl implements J2534 {
    ...
    */
    ...
    public byte[] readVehicleResponse(int channelId, long timeout) {
        long end = currentTimeMillis() + timeout;
        do {
            long remaining = end - currentTimeMillis();
            if (remaining <= 0L) {
                throw new J2534Exception(
                        "readVehicleResponse error: timeout expired waiting for a vehicle response");
            }
            PASSTHRU_MSG msg = doReadMsg(channelId, remaining);
            if (LOGGER.isTraceEnabled())
                LOGGER.trace("Read vehicle Msg: " + toString(msg));
            if (isVehicleResponse(msg)) return data(msg);
        } while (true);
    }
    ...
    private boolean isVehicleResponse(PASSTHRU_MSG msg) {
        return msg.timestamp.intValue() != 0
                && RxStatus.get(msg.rxStatus.intValue()) == RxStatus.RX_INDICATION;
    }
