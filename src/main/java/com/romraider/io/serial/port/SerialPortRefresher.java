...
public final class SerialPortRefresher implements Runnable {
    ...
    private final Object refreshWait = new Object();
    ...
    /** Stops this daemon after its current refresh or sleep completes. */
    public void stop() {
        running = false;
        synchronized (refreshWait) {
            refreshWait.notifyAll();
        }
    }

    private void waitForRefreshInterval() {
        synchronized (refreshWait) {
            if (!running) return;
            try {
                refreshWait.wait(PORT_REFRESH_INTERVAL);
